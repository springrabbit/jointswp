...
const LOCAL_URL = 'https://kitdevmeow.ml/';
...
const SOURCE = {
  ...
  timeline: ['assets/scripts/timeline/**/*.js'],
...
gulp.task('timeline', async function() {

  const CUSTOMFILTER = filter(ASSETS.scripts + 'js/**/*.js', {restore: true});

	return gulp.src(SOURCE.timeline)
		.pipe(plugin.plumber(function(error) {
            gutil.log(gutil.colors.red(error.message));
            this.emit('end');
        }))
		...
		.pipe(CUSTOMFILTER)
			...
			.pipe(plugin.jshint.reporter('jshint-stylish'))
			.pipe(CUSTOMFILTER.restore)
		.pipe(plugin.concat('timeline.js'))
		.pipe(plugin.uglify())
		.pipe(plugin.sourcemaps.write('.')) // Creates sourcemap for minified JS
		.pipe(gulp.dest(ASSETS.scripts))
		.pipe(touch());
...
gulp.task('watch', function() {
	...
	// Watch scripts files
	gulp.watch(SOURCE.scripts, gulp.parallel('scripts', 'timeline'));

  gulp.watch(SOURCE.timeline, gulp.parallel('timeline'));
...
gulp.task('default', gulp.parallel('styles', 'scripts', 'timeline'));
